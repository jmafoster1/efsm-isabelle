...
lemma ltl_step_none:
"possible_steps e s r (fst ie) (snd ie) = {||} \<Longrightarrow> ltl_step e (Some s) r ie = (None, [], r)"
...
lemma state_eq_holds:
"state_eq s = holds (\<lambda>x. statename x = s)"
  apply (rule ext)
...
lemma state_none_2:
"(state_eq None) (make_full_observation e s r p t) \<Longrightarrow> (state_eq None) (make_full_observation e s r p (stl t))"
  by (simp add: state_eq_def)

lemma decompose_pair:
"e \<noteq> (l, i) = (\<not> (fst e =l \<and> snd e = i))"
...
lemma alw_ev:
"alw f = not (ev (\<lambda>s. \<not>f s))"
  by simp

lemma stream_eq_scons:
"shd x = h \<Longrightarrow> stl x = t \<Longrightarrow> x = h##t"
  by auto
...
lemma alw_holds:
"alw (holds P) (h##t) = (P h \<and> alw (holds P) t)"
  apply standard
   apply auto[1]
  by (metis alw.simps holds_Stream stream.sel(2))

lemma alw_holds2:
"alw (holds P) ss = (P (shd ss) \<and> alw (holds P) (stl ss))"
  by (meson alw.simps holds.elims(2) holds.elims(3))

lemma snth_sconst:
"(\<forall>i. s !! i = h) = (s = sconst h)"
...
lemma alw_sconst:
"(alw (\<lambda>xs. shd xs = h) t) = (t = sconst h)"
...
"(p suntil q) \<omega> \<Longrightarrow> \<exists>j. q (sdrop j \<omega>) \<and> (\<forall>k < j. p (sdrop k \<omega>))"
...
"ev \<psi> \<omega> \<Longrightarrow> (\<phi> until \<psi>) \<omega> \<Longrightarrow> (\<phi> suntil \<psi>) \<omega>"
...
"ev (not \<phi>) \<omega> \<Longrightarrow> (\<phi> until \<psi>) \<omega> \<Longrightarrow> ev \<psi> \<omega>"
proof(induction rule: ev.induct)
  case (base xs)
  then show ?case
...
lemma sdrop_until:
"q (sdrop j \<omega>) \<Longrightarrow> \<forall>k<j. p (sdrop k \<omega>) \<Longrightarrow> (p until q) \<omega>"
...
lemma sdrop_suntil:
"q (sdrop j \<omega>) \<Longrightarrow> (\<forall>k < j. p (sdrop k \<omega>)) \<Longrightarrow> (p suntil q) \<omega>"
